...
public class WrongQuestionServiceImpl implements WrongQuestionService {
    ...
    @Override
    public WrongQuestionPageResponseVo list(int page, int size) {
        ...
        if (total == 0) {
            return new WrongQuestionPageResponseVo(0, page, size, Collections.emptyList());
        }
        int offset = (page - 1) * size;
        List<WrongQuestionItemResponseVo> list = wrongQuestionMapper.findPageByUserId(userId, offset, size);
        return new WrongQuestionPageResponseVo(total, page, size, list);
    }
